...
// import { useInvestments } from "@/lib/tenstack-hooks/allInvestments";
...
type InvestmentResponse = Investment & {
  user: User;
  ...
  const columns: ColumnDef<InvestmentResponse, unknown>[] = [
    {
      ...
      cell: ({ row }: { row: Row<InvestmentResponse> }) => (
    ...
    //     ),
    //   },
  ].filter(Boolean) as ColumnDef<InvestmentResponse, unknown>[]; // Ensure no null values and assert the correct type
